...
describe('a narrowing revokes the keys the new role could not mint', () => {
  ...
  it('names a key the second pass could not revoke, since nothing retries it', async () => {
    // The role is written by now, and the same PATCH answers a request for the
    // role the member already holds without touching anything.
    stubMemberKeys({ permissions: ['read', 'DeleteBucket'] });
    mockDeleteAccessKey.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error('down'));
    // The stub cannot delete rows, so the second pass sees a second key.
    ddbMock
      .on(QueryCommand, {
        TableName: 'UserInfoTable',
        ExpressionAttributeValues: {
